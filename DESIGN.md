...
- **Sidebar:** a collapsible floating card (258px expanded, 96px as an icon rail) on the `bg` plane — inset from the viewport with a `p-3`/`gap-3` gutter rather than flush, `{rounded.md}` (14px), `shadow-floating`. A glass collapse toggle straddles the trailing edge, vertically centered, faintly visible at rest (not fully hidden — a hover-only reveal made it too easy to miss/mis-click). The full "Eve." wordmark sits at the top with a `26px` bottom margin — small, gradient-toned and centered (`text-[30px]`) in the collapsed rail; expanded is flat two-tone (`text-[28px]`, solid `accent` word, `indigo-500` dot, no gradient fill) with an English "EVENT STUDIO" overline underneath (Space Grotesk, `10px`, `3px` tracking, `#b6b2c4`) — a venue-switcher dropdown (circular venue mark, initials or logo) sits directly beneath that. Nav items, the bottom profile card, and the venue switcher share the *same* `{rounded.md}` (14px) as the outer card — never a full pill even under hover/active fill. Active nav is bold (700) `accent` on `accent-tint`; inactive is semibold (600) `muted`, hovering to `accent-tint`. The profile card carries its own hairline `border` and a recessed `inset` fill (not the accent tint — that's its hover state), with a `grad-cta`-gradient avatar disc.
- **Top bar (app shell):** a floating card matching the sidebar exactly — `{rounded.md}` (14px), `shadow-floating`, same `gap-3` gutter to the sidebar and to `main` below it. Title only, no subtitle line (removed everywhere — the page title alone carries the header). The shared `SearchInput` component ("חיפוש לקוח או אירוע") sits centered between the title and the action group — same bordered-white, muted-placeholder field as every other search box in the app, not a one-off style. The action group is icon buttons (venue shortcut, notifications) beside the primary CTA — the one saturated surface here, and the richest gradient outside the mesh itself: a `150deg` purple base (the `grad-cta` recipe) layered with a glass-style specular sheen on top, an `amber` glow at the start corner and a `magenta` glow at the opposite corner (both pulled from the mesh's own accent vocabulary, not new colors), and `.grain` over the top so it doesn't read as flat UI chrome — an opaque, small-scale echo of the mesh treatment for a surface that never sits over the mesh itself. `text-canvas` label, kept in its own `z-10` layer so the grain never sits over the text.
